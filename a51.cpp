...
bool A51(char*filename1,char*filename2,char*key){//поточный шифр А5/1
    ...
    vector<short> z(23);
    short keybyte;
    vector<short>keystr(64);//массив на 64 бита ключа
    int j=0;
    while(keyfile.read((char *)&keybyte, sizeof(short))){
    ...
    while(file.read((char *)&byte,sizeof(short))){
        short majorbit;//преобладающий среди синхронизационных бит (единица или ноль)
        ...
        if(x[8] == majorbit){//если синхронизационный бит совпал с majorbit - сдвигаем первый регистр
            short bit = x[13]^x[16]^x[17]^x[18];//получаем бит через многочлен обратной связи первого регистра
        ...
        if(y[10] == majorbit){//если синхронизационный бит совпал с majorbit - сдвигаем второй регистр
            short bit = y[20]^y[21];//получаем бит через многочлен обратной связи второго регистра
        ...
        if(z[10] == majorbit){//если синхронизационный бит совпал с majorbit - сдвигаем третий регистр
            short bit = z[7]^z[20]^z[21]^z[22];//получаем бит через многочлен обратной связи третьего регистра
        ...
        short resbyte = byte^x[18]^y[21]^z[22];
